...
--	print('store name '..tostring(alert))
	TriggerEvent("vorp:TipBottom", 'Telegrama de robo en curso en ' .. alert, 15000)
...
AddEventHandler('wcrp_robbery:startAnimation', function()	
	...
	exports['progressBars']:startUI(Config.LockpickTime, "Ganzuando la Caja rejistradora...")
    Citizen.Wait(15000)
...
AddEventHandler("wcrp_robbery:startTimer",function(robtimer)
	...
	---for i = 1, #Config.Shops do
		TriggerEvent("wcrp_robbery:startTimers")
			while timers do
			Citizen.Wait(0)
			DrawTxt("Asegure la zona para... "..timerCount.." segundos", 0.50, 0.85, 0.7, 0.7, true, 255, 255, 255, 255, true)
			local playerPed = PlayerPedId()
